...
```
Next.js Frontend ◄──HTTP/WS──► FastAPI Server ◄──BLE──► HC-02 ◄──UART──► Arduino UNO
   :3000                          :8000                                    Robot Car
```
...
- Xe robot **bật nguồn** (module HC-02 phải hoạt động)
...
| `POST` | `/api/connect` | Kết nối BLE tới HC-02 |
...
Sửa file `config.py`:
- `BLE_DEVICE_ADDRESS`: MAC address HC-02 (để `None` sẽ auto-scan)
- `BLE_DEVICE_NAME`: Tên thiết bị để scan (mặc định: `"HC-02"`)
